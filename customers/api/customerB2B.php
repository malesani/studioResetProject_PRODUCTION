...
try {
    ...
    switch ($method) {
            ...
            switch ($requestData['opt']) {
                case 'customer_info':
                    if (empty($requestData['customer_uid'])) {
                        http_response_code(400);
                        echo json_encode($authManager->responseArr(['success'=>false,'message'=>$trad->lang('customer.400.missingRequiredParameters'),'error'=>'Missing parameter: customer_uid']));
                        break;
                    }
                    ...
                    if ($info) {
                        ...
                        echo json_encode($authManager->responseArr(['success'=>true,'message'=>$trad->lang('customer.200.successRetriveData'),'data'=>['customer_info'=>$info]]));
                    } else {
                        http_response_code(404);
                        echo json_encode($authManager->responseArr(['success'=>false,'message'=>$trad->lang('customer.400.errorRetriveData'),'error'=>'Empty customer_info']));
                    }
                    break;
                ...
                case 'customers_list':
                    if (false) {
                        http_response_code(400);
                        echo json_encode($authManager->responseArr(['success'=>false,'message'=>$trad->lang('customer.400.missingRequiredParameters'),'error'=>'Missing parameter: ---']));
                        break;
                    }
        ...
        case 'PUT':
            switch ($requestData['opt']) {
                case 'customer_info':
                    ...
                    if ($missing) {
                        http_response_code(400);
                        echo json_encode($authManager->responseArr(['success'=>false,'message'=>$trad->lang('customer.400.missingRequiredParameters'),'error'=>'Missing: '.json_encode(array_values($missing))]));
                        break;
                    }
                    ...
                    echo json_encode($authManager->responseArr(array_merge(['success'=>$res['success'],'message'=>$trad->lang($res['message'])], $res['success'] ? ['data'=>['customer_info'=>$obj->get_customerInfo()]] : ['error'=>$res['error']])));
                    break;
        ...
        default:
            ...
            echo json_encode(['success'=>false,'message'=>$trad->lang('customer.405.methodNotAllowed'),'error'=>'Method "'.$method.'" not allowed']);
    }

    if ($defaultOption) {
        http_response_code(400);
        echo json_encode($authManager->responseArr(['success'=>false,'message'=>$trad->lang('customer.400.invalidOption'),'error'=>'Option: '.$method.' - "'.$requestData['opt'].'" invalid']));
    }
