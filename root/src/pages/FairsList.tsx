...
import {
    APIFair,
    get_fairsListPaginated,
    create_fair,
    update_fair,
    delete_fairByUid,
} from "../api_module/fairs/FairsRequest";
...
const FiereLista: React.FC<ObjectivesListProps> = ({ project_uid }) => {
    ...
    const [selectedFair, setSelectedFair] = useState<APIFair | null>(null);
    ...
    // ⬇️ UPDATE handler
    async function onUpdateFair(
        payload: Partial<APIFair> & { fair_uid: string }
    ) {
        // normalizza active come 0/1 se presente
        const to01 = (v: unknown): 0 | 1 => (v === "1" || v === 1 || v === true ? 1 : 0);
        const normalized =
            payload.active === undefined
                ? payload
                : { ...payload, active: to01(payload.active as any) };

        const res = await update_fair(normalized);
        if (res.response.success) {
            await fetchFairsOnDemand();
        }
        return res;
    ...
    // ⬇️ DELETE handler
    // HANDLERS
    const handleDelete = async (fair_uid: string) => {
        const defaultOpts: SweetAlertOptions = {
            title: "Sei sicuro di voler eliminare questa fiera?",
            icon: "warning",
        ...
        try {
            ...
            const result = await delete_fairByUid!({ fair_uid: fair_uid! });    // invoca la tua API con payload = params + dati univoci dal row.data

            if (result.response.success) {
                setFairsList(rs => rs.filter(r => r.fair_uid !== fair_uid));         // rimuovi localmente
        ...
        <>
                        ...
                        {!loadingMode_fairs && !error_fairs && fairs_list.length > 0 && (
                            <>
                                <div
                                    ...
                                    style={{
                                        // ⬇️ aggiunta colonna AZIONI
                                        gridTemplateColumns: "2fr 1fr 0.7fr 2fr 2fr 1.2fr 0.8fr",
                                        fontSize: "0.9rem",
                                    }}
                                >
                                    <div>NOME</div>
                                ...
                                {fairs_list.map((fiera, i) => (
                                    <MDBCardBody
                                        ...
                                        style={{
                                            // ⬇️ stessa griglia con colonna AZIONI
                                            gridTemplateColumns: "2fr 1fr 0.7fr 2fr 2fr 1.2fr 0.8fr",
                                    ...
                                    >
                                        ...
                                        <div className="d-flex gap-2">
                                            ...
                                            <MDBBtn
                                                ...
                                                color="link"
                                                className="text-muted p-0"
                                                onClick={() => handleDelete(fiera.fair_uid!)}
                                                title="Elimina"
                                            >
            ...
            <MDBModal tabIndex="-1" open={editModalOpen} setOpen={setEditModalOpen}>
                <MDBModalDialog centered size="lg">
                    <MDBModalContent>
                        <MDBModalHeader>
                            <MDBModalTitle>Modifica Fiera</MDBModalTitle>
                        ...
                        <MDBModalBody>
                            {selectedFair && (
                                <GeneralForm<
                                    Partial<APIFair> & { fair_uid: string },
                                    {}
                                >
                                    mode="update"
                                    fields={CreateFair_FormFields as FieldConfig<Partial<APIFair> & { fair_uid: string }>[]}
                                    data={selectedFair}
                                    updateData={onUpdateFair}
                                    onSuccess={() => {
                                        toggleEditModal();
                                        setSelectedFair(null);
                                    }}
                                    updateBtnProps={{ label: "Salva Modifiche", labelSaving: "Salvataggio..." }}
                                />
                            )}
                        </MDBModalBody>
                    </MDBModalContent>
