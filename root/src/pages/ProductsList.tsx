...
import { APIProduct, create_product, update_product, delete_productByArtCode, get_productsListPaginated, get_categoryTree } from '../api_module/products/ProductsRequest';
...
const MaterialiList: React.FC<ObjectivesListProps> = ({ project_uid }) => {
  ...
  const CreateProduct_FormFields: FieldConfig<APIProduct>[] = [
    // ── Info generali ────────────────────────────────────────────────────────────
    {
      ...
      properties: {
        placeholder: "Es. PRD-20250103-001"
      },
      extraElements: [
        {
          position: "before",
  ...
  // HANDLERS
  const handleDelete = async (art_code: string) => {
    ...
    try {
      ...
      const result = await delete_productByArtCode!({art_code: art_code!});    // invoca la tua API con payload = params + dati univoci dal row.data

      if (result.response.success) {
        setProductsList(rs => rs.filter(r => r.art_code !== art_code));         // rimuovi localmente
                      ...
                      >
                        ...
                        <td>
                          <div className="d-flex gap-2">
                            ...
                            <MDBBtn size="sm" color="link" className="text-muted p-0" onClick={()=>{handleDelete(p.art_code)}}>
