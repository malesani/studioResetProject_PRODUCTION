...
require_once __DIR__ . "/../obj/conceptsObj.php";
...
try {
    ...
    if (empty($requestData['opt'])) {
        ...
        echo json_encode($authManager->responseArr([
            'success' => false,
            'message' => $trad->lang('concepts.400.optionNotSet'),
    ...
    switch ($method) {
            ...
            switch ($requestData['opt']) {
                case 'concept_data':
                    if (empty($requestData['project_uid'])) {
                        ...
                        echo json_encode($authManager->responseArr([
                            'success' => false,
                            'message' => $trad->lang('concepts.400.missingRequiredParameters'),
                            'error'   => 'Missing parameter: project_uid'
                        ]));
                        break;
                    }

                    $obj  = new conceptsObj($authManager, $permsManager, $requestData['project_uid']);
                    $info = $obj->get_conceptData();
                    http_response_code(200);
                    echo json_encode($authManager->responseArr([
                        'success' => true,
                        'message' => $trad->lang('concepts.200.successRetrieveData'),
                        'data'    => ['concept_data' => $info]
                    ]));
                    break;
        ...
        case 'PUT':
            switch ($requestData['opt']) {
                case 'concept_data':
                    ...
                    $obj  = new conceptsObj($authManager, $permsManager, $requestData['project_uid']);
                    $res = $obj->set_conceptData($requestData['list_concept_uid']);
                    ...
                    if ($res['success']) {
                        ...
                        $extra = ['data' => ['concept_data' => $obj->get_conceptData()]];
                        http_response_code(200);
                    } else {
        ...
        default:
            ...
            echo json_encode([
                'success' => false,
                'message' => $trad->lang('concepts.405.methodNotAllowed'),
    ...
    if ($defaultOption) {
        ...
        echo json_encode($authManager->responseArr([
            'success' => false,
            'message' => $trad->lang('concepts.400.invalidOption'),
