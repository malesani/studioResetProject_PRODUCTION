...
export function GeneralTable<T extends Record<string, any>, P extends Record<string, any>, F extends Record<string, any> = DefaultFilters>(props: GeneralTableProps<T, P, F>) {
    ...
    const controlledMetaData: TableMetadata | undefined = !isFetchMode ? (props as InitialProps<T>).meta : undefined;
    ...
    useEffect(() => {
        ...
        // ✅ Aggiorno metadati se presenti
        if (meta && (meta.total !== undefined || meta.per_page !== undefined)) {
            const total = Number(meta.total ?? data.length);
            const perPage = Number(meta.per_page ?? 25);
            setTotalPages(Math.max(1, Math.ceil(total / perPage)));
        } else {
            // fallback in caso non ci siano metadati
            setTotalPages(1);
        }
