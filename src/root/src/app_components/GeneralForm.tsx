...
import {
  ...
  computeValidation,
  RepeaterFieldConfig
} from '../app_components/GeneralForm_Common';

import { useDependentOptions, DependentConfig } from '../app_components/useDependentOptions';
...
import { SelectData } from "mdb-react-table-editor/dist/TableEditor/DatatablePagination/types";

export type { FieldConfig, SelectData } from '../app_components/GeneralForm_Common';
...
export function GeneralForm<T extends Record<string, any>, P extends Record<string, any> = {}>(props: GeneralFormProps<T, P>) {
  ...
  useEffect(() => {
    selectConfigs.forEach((selectConf) => {
      ...
      const dependencies = selectConf.dependencies;
  ...
  return (
    ...
    >
      ...
      {!hideHeader && (<>
        ...
        {showHeaderDivider && <hr className="mt-0" />}
        ...
        : <MDBRow>
          {flow.fields
            .filter(f => {
              ...
              const beforeElems = field.extraElements?.filter(e => e.position === "before") ?? [];
              const afterElems = field.extraElements?.filter(e => e.position === "after") ?? [];
              ...
              return (
                <React.Fragment key={String(key)}>
                  {field.hrBefore && <hr className="mt-0" />}
                  ...
                  <MDBCol
                    className={"mb-3 " + (field.class_name || '')}
                  ...
                  {field.hrAfter && <hr className="mt-0" />}
