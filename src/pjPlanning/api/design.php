...
require_once __DIR__ . "/../obj/designObj.php";
...
try {
    ...
    if (empty($requestData['opt'])) {
        ...
        echo json_encode($authManager->responseArr([
            'success' => false,
            'message' => $trad->lang('design.400.optionNotSet'),
    ...
    switch ($method) {
            ...
            switch ($requestData['opt']) {
                case 'design_data':
                    if (empty($requestData['project_uid'])) {
                        ...
                        echo json_encode($authManager->responseArr([
                            'success' => false,
                            'message' => $trad->lang('design.400.missingRequiredParameters'),
                            'error'   => 'Missing parameter: project_uid'
                        ]));
                        break;
                    }

                    $obj  = new designObj($authManager, $permsManager, $requestData['project_uid']);
                    $info = $obj->get_designData();
                    http_response_code(200);
                    echo json_encode($authManager->responseArr([
                        'success' => true,
                        'message' => $trad->lang('design.200.successRetrieveData'),
                        'data'    => ['design_data' => $info]
                    ]));
                    break;
        ...
        case 'PUT':
            switch ($requestData['opt']) {
                case 'design_data':
                    ...
                    $obj  = new designObj($authManager, $permsManager, $requestData['project_uid']);
                    $res = $obj->set_designData($requestData['list_design_uid']);
                    ...
                    if ($res['success']) {
                        ...
                        $extra = ['data' => ['design_data' => $obj->get_designData()]];
                        http_response_code(200);
                    } else {
        ...
        default:
            ...
            echo json_encode([
                'success' => false,
                'message' => $trad->lang('design.405.methodNotAllowed'),
    ...
    if ($defaultOption) {
        ...
        echo json_encode($authManager->responseArr([
            'success' => false,
            'message' => $trad->lang('design.400.invalidOption'),
